...
## NAM Captures (optional AMP/CAB replacement) — DISABLED as of 2026-09-18

**Do not use NAMs in new patches right now.** Michael is hitting a volume issue with the N->S slot on the Valeton device itself (device-side bug, not a tooling problem) and wants NAMs off until it's resolved. Default every patch to a real GP-5 `AMP` model plus an `IRs/ir.md` capture for `CAB` (see "IR Cab Captures" below) instead. The rest of this section (`NAMs/nams.md`, the JSON `"nam"` field, the encoder's slot handling) stays in place for when NAMs come back — just don't reach for it meanwhile.
