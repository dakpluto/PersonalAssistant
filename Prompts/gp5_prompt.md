...
2. Read the files under /Modules/ to learn all the options under each module, their settings, and what they are designed to model. If Full Board = True, also read the files under /Pedals/ to learn the real controls on every other pedal in the chain — don't guess at knobs/switches that aren't documented there. Also read `NAMs/snaptone_combos.md` for the 30 NAM+IR snaptones loaded on the device as an alternative to the AMP/CAB modules, and `IRs/ir.md` for the list of IR cab captures available as an alternative to the CAB module — see "NAM Captures" and "IR Cab Captures" below for how and when to use each.
...
10. Push the patch to my music site (`dakplutomusic.us`) automatically — every patch built in this repo, no need to ask first:
    ...
    - **One site piece per song, not per instrument.** Guitar and bass patches for the same song go on a single piece. Upload both under the song-level title ("When Wind Meets Fire — Elevation Worship"), dropping any "(Rhythm Guitar)"/"(Lead Guitar)" suffix from the H1. If the song already has a piece from the other instrument, even under a slightly different title (e.g. "Higher - Creed" with a hyphen), reuse that exact existing title. The API keeps files apart by filename, so `<Song>-RhythmGtr.prst` and `<Song>.prst` coexist. It also unions tags, and fills `notes` only when the piece has none. When adding the second instrument, PATCH the piece's notes to `Guitar: … | Bass: …` afterward.
...
- Clean-vs-dirty CTL switching still comes from PRE/DST. The snaptone is one fixed amp voice for the whole patch, so pick the combo that suits the CTL-off sound and let PRE/DST add the push.
- Weighting: bass and lower-gain/edge-of-breakup guitar get strong preference for a snaptone. Use one by default when a combo fits. High-gain guitar keeps the conservative rule: default to the GP-5's own AMP/CAB unless a combo is a genuinely close match for the real amp (e.g. the Rectifier combos for a Rectifier song).
- Prefer the GP-5's own AMP (plus an IR per "IR Cab Captures") when it's a direct model of the real amp and the closest combo is only a stand-in. Examples: Mess Bass for Myung's Mesa, Solo100 for a Soldano, J-120 CL for a JC-120, Bellman 59N for a '59 Bassman. Same if the patch's CTL-off part is clean and the only matching combo is a crunch capture.
...
- Optional top-level `nam` field documents a snaptone used in place of AMP/CAB (see "NAM Captures" above): the snaptone name, its source NAM + IR, and its slot (1-80) from `NAMs/snaptone_combos.md`, plus its Gain/VOL/Bass/Middle/Treble settings. Omit it when a patch doesn't use one. When present, `AMP` and `CAB` must both be `"model": null` in `modules`. Always include `"slot"`: the encoder then writes a real, active `N->S` block for that slot. (Without `slot` the field is informational only and N->S stays inactive; that only applies to a NAM that isn't loaded, which the current rules don't allow.)
